...
const Settings = (() => {
  function render() {
    ...
    main.innerHTML = '<h1 class="vh">設定</h1><div class="card">' +
      ...
      '<h3>毎日の流れ</h3><p>1. 「今日」を開く → 教科ごとの予定が出ます。<b>開く ›</b> で教材にジャンプ。<br>2. 要点を読む → 一問一答・演習を解く → <b>答え</b> を見て <b>○△×</b> を自分でつける。<br>3. 終わったら「今日」のチェックを入れる（または教材の上の「この予定を完了にする」）。</p>' +
      ...
      '<h3>直しリスト</h3><p>×や△をつけた問題は「進捗 → 直しリスト」に自動で集まります。<b>カードで解き直す</b> で一気に復習し、○に直せばリストから消えます。</p>' +
  ...
  function backupJSON() {
    return JSON.stringify({ app: 'hokushin-note', v: 2, exported: new Date().toISOString(), progress: S.progress, plan: S.plan, notes: S.notes, settings: S.settings, hl: S.hl, ink: S.ink, nb: S.nb });
  }
  ...
  async function importBackup(text) {
    ...
    if (!obj || obj.app !== 'hokushin-note' || !obj.progress) { toast('このアプリのバックアップではないようです'); return; }
